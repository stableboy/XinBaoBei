...
begin
	...
	*/
	...
	-- 更新父目录ID
	drop table if exists tmp_menu2
	;
	create table tmp_menu2 
	as select * from T_Menu
	;
	...
	where 1=0
		...
		-- 有.但是没有父节点
		or (Sequence like '%.%' and ParentMenu not in (select parent.ID  from tmp_menu2 parent)  ) 
	;
